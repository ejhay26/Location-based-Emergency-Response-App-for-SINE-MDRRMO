...
class EmergencyController extends Controller
{
    ...
    {
        $tokens = DB::table('device_tokens')->where('user_id', $userId)->pluck('token')->toArray();
        if (!empty($tokens)) (new FirebasePushService())->send($tokens, $title, $body, $data);
    }
    ...
    {
        $request->validate(['request_id' => 'required|integer']);
        $emergency = DB::table('emergency_requests')->where('request_id', $request->request_id)->first();
        ...
        DB::table('users')->where('user_id', $emergency->user_id)->increment('false_alarm_strikes');
        $user    = DB::table('users')->where('user_id', $emergency->user_id)->first();
        $strikes = $user->false_alarm_strikes;
        if ($strikes >= 3) {
            DB::table('users')->where('user_id', $emergency->user_id)->update([
    ...
    public function resolveHazard(Request $request)
    {
        ...
        DB::table('hazards')->where('hazard_id', $request->hazard_id)->update(['status' => 'Resolved', 'updated_at' => now()]);
