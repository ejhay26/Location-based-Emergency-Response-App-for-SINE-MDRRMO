...
<nav class="ctab-bar">
  <div class="ctab-indicator" [style.left]="'calc(' + (activeIndex * 20) + '% + 4px)'"></div>
  ...
  `,
  styles: [`
    :host { display: block; padding-bottom: calc(96px + env(safe-area-inset-bottom, 0px)); }
    .ctab-bar {
      position: fixed; bottom: calc(16px + env(safe-area-inset-bottom, 0px));
      left: 16px; right: 16px; height: 60px;
      background: var(--ion-color-danger); border-radius: 30px;
      display: flex; align-items: center; z-index: 9000;
      box-shadow: 0 6px 28px rgba(235, 68, 90, 0.45);
      user-select: none; -webkit-user-select: none;
    }
    .ctab-indicator {
      position: absolute; top: 5px; height: 50px; width: calc(20% - 8px);
      background: rgba(255,255,255,0.22); border-radius: 25px;
      transition: left 0.28s cubic-bezier(0.4,0,0.2,1); pointer-events: none;
    }
    .ctab-btn {
      flex: 1; display: flex; flex-direction: column; align-items: center;
      justify-content: center; gap: 3px; background: transparent; border: none;
      color: rgba(255,255,255,0.5); cursor: pointer; height: 100%; padding: 0;
      transition: color 0.2s ease; -webkit-tap-highlight-color: transparent;
    }
    .ctab-btn.active { color: white; }
    .ctab-btn i { font-size: 19px; }
    .ctab-label { font-size: 10px; font-weight: 600; letter-spacing: 0.3px; line-height: 1; }
    .ctab-avatar {
      width: 26px; height: 26px; border-radius: 50%;
      background-size: cover; background-position: center;
      border: 2px solid rgba(255,255,255,0.45); flex-shrink: 0; transition: border-color 0.2s;
    }
